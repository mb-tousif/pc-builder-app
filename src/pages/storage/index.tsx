...
export default function Storage({ products }: { products: TProducts[] }) {
    return (
      <RootLayout>
        <h1 className="text-2xl sm:text-4xl text-center p-3 sm:font-bold text-gray-50">
          Storage Devices Feature Products
        </h1>
  ...
  export async function getStaticProps() {
    try {
      ...
      const products = data.filter((product) => product.category === ECategory.Storage_Device);
      return {
        props: {
          products,
        },
